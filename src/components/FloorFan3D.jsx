...
// ─── Blade shape: straight wide rectangular blade matching real product ───────

let _bladeShape = null;
function getBladeShape() {
  if (_bladeShape) return _bladeShape;

  // Real product: wide, straight, flat rectangular blades
  const rootHW = 0.30;     // wide at hub root (matching real product proportions)
  const tipHW = 0.24;      // slight taper toward tip
  const len = 2.05;        // reach ~92% of cage radius (2.22)
  const tipR = 0.06;       // small rounded tip
  ...
  // Rounded tip
  s.bezierCurveTo(
    -tipHW * 0.4, len + tipR * 0.2,
    tipHW * 0.4, len + tipR * 0.2,
    tipHW, len - tipR,
  );

  // Straight right edge
  s.lineTo(rootHW, 0.44);
...
function WireGuard({ zPos, ringCount = 14, outerR = 2.22, innerR = 0.5 }) {
  ...
  return (
    <group position={[0, 0, zPos]}>
      ...
      {/* 8 radial bars — from center to outer edge */}
...
function RotatingAssembly({ speed }) {
  const groupRef = useRef();
  const smoothSpeed = useRef(speed ?? 90);

  // Map RPM to visual rotation rate — ~48% of real physics speed
  // 55 RPM → 2.75 rad/s, 125 RPM → 6.25 rad/s
  const toRate = (rpm) => rpm * 0.05;
  ...
  useFrame((_, delta) => {
    ...
    if (speed != null) {
      ...
      groupRef.current.rotation.z -= delta * toRate(smoothSpeed.current);
    } else {
...
function FloorFanAssembly({ speed }) {
  return (
...
export default function FloorFan3D({ speed }) {
  return (
    <div
      role="img"
